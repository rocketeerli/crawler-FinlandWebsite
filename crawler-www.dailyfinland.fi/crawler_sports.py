...
def get_html(url) :
    ...
    for i in soup.find_all('a', href=re.compile("http://www.dailyfinland.fi/sports/")) :
        yield i.get('href')
# 获取超链接页面详细信息
...
def save_data(content_list) :
    with open('../www.dailyfinland.fi/sports.json', 'a', encoding='utf-8') as f:
...
# 主函数
def main() :
    url = 'http://www.dailyfinland.fi/sports'
    fun_call(url)
