...
def get_html(url) :
    ...
    for i in soup.find_all('a', href=re.compile(r"https://finland.fi/christmas/.*")) :
        yield i.get('href')
    for index in range(2, 100) :
        html = requests.get(url + "page/" + str(index))
        if not (html is None) :
            html.encoding='utf-8'
            soup = BeautifulSoup(html.text, 'lxml')
...
def save_data(content_list) :
    with open('../finland.fi/christmas.json', 'a', encoding='utf-8') as f:
...
# 主函数
def main() :
    url = 'https://finland.fi/category/christmas/'
    fun_call(url)
