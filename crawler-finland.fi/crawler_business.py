...
def get_html(url) :
    ...
    for i in soup.find_all('a', href=re.compile(r"https://finland.fi/business-innovation/.*")) :
        yield i.get('href')
...
def save_data(content_list) :
    with open('../finland.fi/business.json', 'a', encoding='utf-8') as f:
...
def main() :
    ...
    url = 'https://finland.fi/category/business-innovation/'
    fun_call(url)
