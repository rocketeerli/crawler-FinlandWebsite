...
def get_html(url) :
    ...
    for i in soup.find_all('a', href=re.compile(r"/working-life/\d{4}[^/]*\.html$")) :
...
def save_data(content_list) :
    with open('../www.helsinkitimes.fi/working-life.json', 'a', encoding='utf-8') as f:
...
# 主函数
def main() :
    url = 'http://www.helsinkitimes.fi/working-life.html'
    fun_call(url)
